...
class ECMPStructuredRouting(StructuredRouting):
    '''ECMP Structured Routing.'''
    ...
    def __init__(self, topo):
        ...
        def choose_ecmp(paths, src, dst, hash_):
            '''Choose ECMP based path

            @param path paths of dpids generated by a routing engine
            @param src src dpid
            @param dst dst dpid
            '''
            print('num_paths: ' + str(len(paths)))
            m = md5.new()
	    m.update(src)
            m.update(dst)
            index = int(m.hexdigest(), 16) % len(paths) 
            lg.info('INSIDE ECMP')
            print('src: ' + src + ', dst: ' + dst + ' -> ' + str(paths[index]))
            return paths[index]
           
        super(ECMPStructuredRouting, self).__init__(topo, choose_ecmp)

class RPSStructuredRouting(StructuredRouting):
    '''RPS Structured Routing.'''
    ...
    def __init__(self, topo):
        ...
        def choose_rps(paths, src, dst, hash_):
            '''Choose RPS based path

            @param path paths of dpids generated by a routing engine
            @param src src dpid
            @param dst dst dpid
            @param hash_ hash_ ununsed
            '''
            key = src + dst
            if key in counter:
                counter[key] += 1
                ...
                counter[key] = 0
            choice = counter[key] % len(paths)
            return paths[choice]

        super(RPSStructuredRouting, self).__init__(topo, choose_rps)

class FSStructuredRouting(StructuredRouting):
    '''FS Structured Routing.'''
    ...
    def __init__(self, topo):
        ...
        def choose_fs(paths, src, dst, hash_):
            '''Choose FS based path
        ...
        super(RFSStructuredRouting, self).__init__(topo, choose_fs)
